...
<html lang="en">
    ...
    <body>
        ...
        <div class="dashboard-container container">
            <div class="dashboard-card">
                ...
                <div class="top-bar">
                    <h4>CampusConnect</h4>
                    <div>
                        <a href="dashboard.jsp" class="btn btn-sm btn-outline-light me-2">
                ...
                <!-- Requests table - SERVLET DATA FROM DATABASE -->
                <div class="requests-container fade-in">
                    <table class="requests-table">
                        ...
                        <tbody id="requestsTableBody">
                            <!-- SERVLET: Populate from database using JSTL -->
                            <c:choose>
                                <c:when test="${not empty requestScope.userRequests}">
                                    <c:forEach items="${requestScope.userRequests}" var="request">
                                        <tr data-status="${fn:toLowerCase(request.status)}">
                                            <td><strong>${request.supportType}</strong></td>
                                            <td>${request.module}</td>
                                            <td><fmt:formatDate value="${request.createdAt}" pattern="dd/MM/yyyy"/></td>
                                            <td>
                                ...
                                <c:otherwise>
                                    ...
                                    <tr>
                                        <td colspan="5" style="text-align: center; padding: 60px 20px;">
                                            ...
                                            <h4 style="color: #ccc;">No requests found</h4>
                                            <p style="color: #888;">You don't have any requests yet.</p>
                                            <a href="RequestSupport.jsp" class="action-btn mt-3">
        ...
        <script>
                    // SERVLET: Pass request data to JavaScript
                    const requestsData = {
            <c:forEach items="${requestScope.userRequests}" var="req" varStatus="loop">
                    "${req.id}": {
                    id: "${req.requestId}",
                            type: "${req.type}",
                            subject: "${req.module}",
            ...
            document.querySelectorAll('.filter-btn').forEach(b = > b.classList.remove('active'));
                    ...
                    const rows = document.querySelectorAll('#requestsTableBody tr');
                            ...
                            // If no rows visible and not showing empty state row, show message
                            if (visibleCount === 0) {
                    // Check if we already have an empty state row
                    const emptyStateRow = document.querySelector('#requestsTableBody tr.empty-state-row');
                            if (!emptyStateRow) {
                    ...
                    const tbody = document.getElementById('requestsTableBody');
                            ...
                            < i class = "fas fa-clipboard-list" style = "font-size: 48px; color: #444; margin-bottom: 20px;" > < /i>
                            < h4 style = "color: #ccc;" > No requests found < /h4>
                            < p style = "color: #888;" > You don't have any requests matching the selected filter.</p>
                            < a href = "${pageContext.request.contextPath}/CreateRequestServlet" class = "action-btn mt-3" >
                            < i class = "fas fa-plus me-1" > < /i> Create New Request
                            < /a>
                            < /td>
                            `;
            ...
            editBtn.style.display = 'block';
                    ...
                    window.location.href = '${pageContext.request.contextPath}/EditRequestServlet?id=' + requestId;
                    };
            } else {
